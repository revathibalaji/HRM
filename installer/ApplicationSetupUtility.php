...
public static function createDB() {
	...
	} elseif ($_SESSION['dbCreateMethod'] == 'new') { // If the user wants FixNixHRM to create the database for him
...
public static function createUser() {
	...
	if(!@mysql_select_db($_SESSION['dbInfo']['dbName'])) {
		$_SESSION['error'] = 'Unable to access FixNixHRM Database!';
		return;
	}
	...
	if(!mysql_query($query)) {
		$_SESSION['error'] = 'Unable to Create FixNixHRM Admin User Account';
		return;
	}
...
public static function writeLog() {
	...
	$Content .= "FixNixHRM Installation Log\n\n";
...
public static function install() { 
   if (isset($_SESSION['INSTALLING'])) {
	switch ($_SESSION['INSTALLING']) {
		...
		case 4	:	error_log (date("r")." Create FixNixHRM user - Starting\n",3, "installer/log.txt");
					self::createUser();
					error_log (date("r")." Create FixNixHRM user - Done\n",3, "installer/log.txt");
					if (!isset($error) || !isset($_SESSION['error'])) {
						$_SESSION['INSTALLING'] = 5;
						error_log (date("r")." Create FixNixHRM user - No Errors\n",3, "installer/log.txt");
					} else {
						error_log (date("r")." Create FixNixHRM user - Errors\n",3, "installer/log.txt");
						error_log (date("r")." ".(isset($error)? $error: $_SESSION['error'])."\n",3, "installer/log.txt");
					}
					break;
